 ...
 * @returns {Promise<void>}
 */
module.exports.renderArticleDetail = async (req, res, article, userId, {errors = [], commentPosted = ''} = {}) => {
  const page = parseInt(req.params.page) || 1;
...
module.exports.getUserMention = async (content) => {
  ...
  for (mention of mentions) {
    ...
    const nickName = mention.replace('@', '');
    const user = await User.find({nick_name: nickName});
